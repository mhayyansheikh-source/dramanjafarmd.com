            ...
            <Card.Body className="p-4 p-md-5">
                ...
                <div className="animate__animated animate__fadeIn">
                  ...
                  <Row className="gy-3">
                    {["Annual Physical", "Sick Visit", "Chronic Care Follow-up", "Vaccination", "Prescription Refill", "Other"].map((reason, index) => (
                      <Col md={6} key={index}>
                        <div 
                          className={`border rounded-3 p-3 cursor-pointer text-center transition-all ${formData.reason === reason ? 'border-primary bg-primary bg-opacity-10' : 'hover-bg-light'}`}
              ...
              {step === 2 && (
                <div className="animate__animated animate__fadeIn">
                  ...
                  <div className="d-flex justify-content-between mt-5">
                    <Button variant="outline-secondary" className="px-4 rounded-pill fw-semibold" onClick={prevStep}>
                      Back
                    </Button>
                    <Button 
                      variant="primary" 
                    ...
                    <Row className="gy-4">
                      <Col md={6}>
                        <Form.Group>
                          ...
                          <Form.Control type="text" name="firstName" value={formData.firstName} onChange={updateFormData} className="p-3 bg-light border-0" required />
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="fw-semibold text-muted small text-uppercase">Last Name</Form.Label>
                          <Form.Control type="text" name="lastName" value={formData.lastName} onChange={updateFormData} className="p-3 bg-light border-0" required />
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="fw-semibold text-muted small text-uppercase">Email Address</Form.Label>
                          <Form.Control type="email" name="email" value={formData.email} onChange={updateFormData} className="p-3 bg-light border-0" required />
                        </Form.Group>
                      </Col>
                      <Col md={6}>
                        <Form.Group>
                          <Form.Label className="fw-semibold text-muted small text-uppercase">Phone Number</Form.Label>
                          <Form.Control type="tel" name="phone" value={formData.phone} onChange={updateFormData} className="p-3 bg-light border-0" required />
                        </Form.Group>
                      </Col>
                      <Col md={12}>
                        <Form.Group>
                          <Form.Label className="fw-semibold text-muted small text-uppercase">Date of Birth</Form.Label>
                          <Form.Control type="date" name="dob" value={formData.dob} onChange={updateFormData} className="p-3 bg-light border-0" required />
                        </Form.Group>
                      </Col>
                    </Row>
              ...
              {step === 4 && (
                <div className="text-center py-5 animate__animated animate__fadeIn">
                  ...
                  <Button variant="outline-primary" className="mt-4 px-4 rounded-pill" onClick={() => window.location.href = "/"}>
                    Return Home
                  </Button>
                </div>
              )}
